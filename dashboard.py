import sys
from PyQt6.QtWidgets import QFrame, QVBoxLayout, QLabel,QWidget,QMainWindow,QHBoxLayout,QListWidget,QApplication,QScrollArea,QGridLayout,QListWidgetItem, QSizePolicy, QPushButton, QInputDialog, QMessageBox
...
from meal_plan_widget import MealPlanWidget

class ActivityItem(QWidget):
    def __init__(self, date, activity, points, parent=None):
        ...
        date_label.setStyleSheet("font-weight: bold; color: #9E9E9E; min-width: 90px;")
        
        activity_label = QLabel(activity)
        activity_label.setStyleSheet("color: #E0E0E0;")
        activity_label.setSizePolicy(QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Preferred)
        
        points_label = QLabel(f"+{points} pts")
        points_label.setStyleSheet("font-weight: bold; color: #4CAF50; min-width: 60px;")
...
class LeaderboardItem(QWidget):
    def __init__(self, rank, name, points, parent=None):
        ...
        points_label.setObjectName("pointLabel")
...
class Dashboard(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # Create and add the grid layout for the three widgets
        grid_layout = QGridLayout()
        grid_layout.addWidget(self.create_leaderboard(), 0, 0)
        grid_layout.addWidget(self.create_recent_activities(), 1, 0)
        grid_layout.addWidget(self.create_meal_plan(), 0, 1, 2, 1)  # Span 2 rows
    ...
    def create_recent_activities(self):
        ...
        for row in recent_data:
            date_str = row['score_date'].strftime('%Y-%m-%d')
            activity_str = "Workout completed"
            points = row['total_points']
            
            item_widget = ActivityItem(date_str, activity_str, points)
            
            item = QListWidgetItem(activities_list)
            item.setSizeHint(item_widget.sizeHint())
            activities_list.addItem(item)
            activities_list.setItemWidget(item, item_widget)
    ...
    def create_meal_plan(self):
        frame, layout = self.create_widget_frame("7-Day Meal Plan")
        
        self.meal_plan_widget = MealPlanWidget(self.meal_planner.get_meal_plan(), self.meal_planner)
        layout.addWidget(self.meal_plan_widget)

        return frame
    
    def update_meal_plan(self, meal_plan):
        if not meal_plan:
            print("Error: No meal plan data provided")
            return

        # Update the meal plan in the MealPlanWidget
        self.meal_plan_widget.set_meal_plan(meal_plan)  # Remove the empty dictionary
