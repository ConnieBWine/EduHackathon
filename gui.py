...
from meal_plan_extractor import InteractiveMealPlanner
...
class WorkoutApp(QMainWindow):
    def __init__(self):
        ...
        self.central_stacked_widget.addWidget(self.home_screen)
        ...
        self.create_tab_bar()
        self.home_screen.login_successful.connect(self.show_dashboard)
        ...
        self.meal_planner = InteractiveMealPlanner(self.gemini_api_key)
    ...
    def show_dashboard(self, email):
        if self.dashboard is None:
            self.dashboard = Dashboard(self.meal_planner)
    ...
    def process_survey_data(self):
        ...
        # Validate the input
        if not all(self.survey_data.values()):
        ...
        initial_prompt = self.create_initial_prompt(self.survey_data)
        ...
        if self.workout_plan:
            # If we successfully created a workout plan, update the workout tab
        ...
        initial_meal_plan = self.meal_planner.generate_initial_meal_plan(self.survey_data)
        
        # Create and update dashboard with interactive meal planner
        self.dashboard = Dashboard(self.meal_planner)
        self.central_stacked_widget.addWidget(self.dashboard)

        # Start AI response processing in the background
        ...
        self.generate_and_display_meal_plan()

    
    def generate_and_display_meal_plan(self):
        self.add_message("Generating your 7-day meal plan...", False)
        initial_meal_plan = self.meal_planner.generate_initial_meal_plan(self.survey_data)
        self.display_meal_plan(initial_meal_plan)
        self.add_message("Your 7-day meal plan has been created and is displayed in the Dashboard.", False)

        # Update the dashboard with the new meal plan
        if self.dashboard:
            self.dashboard.update_meal_plan(initial_meal_plan)  # Remove the second argument
    ...
    def display_meal_plan(self, meal_plan):
        meal_plan_text = "7-Day Meal Plan:\n\n"
        for day in meal_plan:
            meal_plan_text += f"{day['day']}:\n"
            for meal, food in day['meals'].items():
                meal_plan_text += f"  {meal.capitalize()}: {food}\n"
            meal_plan_text += "\n"
...
# End exercise
    ...
    def create_survey_form(self):
        ...
        activity_layout.setObjectName("activityOption")
        ...
        goal_layout.setObjectName("goalOption")
        ...
        intensity_layout.setObjectName("intensityOption")
    ...
    def show_chat_interface(self):
        # Clear the AI tab layout
        for i in reversed(range(self.ai_tab.layout().count())): 
            self.ai_tab.layout().itemAt(i).widget().setParent(None)

        # Add chat interface elements
        self.ai_tab.layout().addWidget(self.chat_list)
        ...
        self.ai_tab.layout().addLayout(input_layout)
    ...
    def send_message(self, message=None, is_initial_prompt=False):        
        ...
        if message:
            ...
            try:
                ...
                response = f"Error: Unable to get AI response. {str(e)}"

            
            self.add_message(response, False)

            # Only extract workout plan if it's the initial prompt and hasn't been extracted yet
            if is_initial_prompt and not self.initial_plan_extracted:
                ...
                if workout_plan:
                    ...
                    logging.error("Failed to extract or structure workout plan from AI response")
